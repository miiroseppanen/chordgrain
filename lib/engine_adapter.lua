...
function EngineAdapter.load_sample(path)
  ...
  local ok = EngineGlut.load_sample(path)
...
function EngineAdapter.set_continuous(on, midi_note, pos_norm, opts)
  EngineGlut.set_continuous(on, midi_note, pos_norm, opts)
end
